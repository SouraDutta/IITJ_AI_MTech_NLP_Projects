...
class Data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return feature[0], feature[1], feature[2], feature[3], sen_len, feature[4]
...
class Loader():
    ...
    def get_batch_dev_test(self, batch_size, prefix):
        ...
        for i in range(batch_size):
            try:
                sent, triple, pos, char, sen_len, sen = self.loader[prefix].next()
                # print(sen)
            except:
                self.reset(prefix)
                sent, triple, pos, char, sen_len, sen = self.loader[prefix].next()
        ...
        # sen = torch.stack(sen)
        # print(sen)
        return sents, gts, poses, chars, sen_lens, wrapped, sens
...
if __name__ =='__main__':
    ...
    for i in range(1):
        data = data_loader.get_batch_dev_test(10, 'test')
